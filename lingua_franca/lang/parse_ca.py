...
def isFractional_es(input_str):
    ...
    aFrac = {"medio": 2, "mitja": 2, "terç": 3, "quart": 4,
             "quart": 4, "cinquè": 5, "cinquena": 5, "sisè": 6, "sisena": 6,
             "seté": 7, "setena": 7, "vuitè": 8, "vuitena": 8,
             "novè": 9, "novena": 9, "dècim": 10, "desena": 10,
             "onzè": 11, "onzena": 11, "dotzè": 12, "dotzena": 12}

    if input_str.lower() in aFrac:
        return 1.0 / aFrac[input_str]
    if (input_str == "vintè" or input_str == "vintena"):
        return 1.0 / 20
    if (input_str == "trentè" or input_str == "trentena"):
        return 1.0 / 30
    if (input_str == "centè" or input_str == "centèsima"):
        return 1.0 / 100
    if (input_str == "mil·lèsim" or input_str == "mil·lèsima"):
        return 1.0 / 1000
    return False
...
def extract_datetime_es(input_str, currentDate=None, default_time=None):
    def clean_string(s):
        ...
        synonyms = {"matí": ["començar el dia", "aviat", "alba"],
                    "tarda": ["mitja tarda", "vespre", "vesprada"],
                    "nit": ["anochecer", "tarde"]}
        for syn in synonyms:
            for word in synonyms[syn]:
                s = s.replace(" " + word + " ", " " + syn + " ")
        # relevant plurals, cant just extract all s in pt
        wordlist = ["matins", "tardes", "nits", "dies", "setmanas",
                    "anys", "minuts", "segons", "els", "les", "seguents",
                    "pròximes", "pròxims", "hores"]
        for _, word in enumerate(wordlist):
            s = s.replace(word, word.rstrip('s'))
        s = s.replace("mesos", "mes").replace("anteriors", "anterior")
        return s
    ...
    time_indicators = ["al", "la", "al", "pel", "passats",
                       "passades", "dia", "hora"]
    days = ['dilluns', 'dimarts', 'dimecres',
            'dijous', 'divendres', 'dissabte', 'diumenge']
    months = ['gener', 'febrer', 'març', 'abril', 'maig', 'juny',
              'juliol', 'agost', 'setiembre', 'octubre', 'novembre',
              'desembre']
    monthsShort = ['gen', 'feb', 'mar', 'abr', 'mai', 'jun', 'jul', 'ago',
                   'set', 'oct', 'nov', 'des']
    nexts = ["següent", "pròxim", "pròxima"]
    suffix_nexts = ["següents", "subsegüents"]
    lasts = ["últim", "última"]
    suffix_lasts = ["passada", "passat", "anterior", "abans"]
    nxts = ["després", "següent", "próxim", "próxima"]
    prevs = ["abans", "previa", "previ", "anterior"]
    froms = ["desde", "a", "per", "després de", "por", "próxim",
             "próxima", "de"]
    thises = ["aquest", "aquesta"]
    froms += thises
    ...
    for idx, word in enumerate(words):
        ...
        elif (word == "abans-d'ahir" or
              (word == "abans" and wordNext == "d'ahir")) and not fromFlag:
            dayOffset -= 2
            used += 1
            if wordNext == "ayer":
                used += 1
        elif word == "abans" and wordNext == "abans" and wordNextNext == \
                "d'ahir" and not fromFlag:
            dayOffset -= 3
            used += 3
        elif word == "abans abans-d'ahir" and not fromFlag:
            dayOffset -= 3
            used += 1
        # day after tomorrow
        elif word == "passat" and wordNext == "demà" and not fromFlag:
            dayOffset += 2
            used = 2
        # day before yesterday
        elif word == "abans" and wordNext == "d'ahir" and not fromFlag:
            dayOffset -= 2
            used = 2
        ...
        validFollowups.append("abans-d'ahir")
        ...
        if word in froms and wordNext in validFollowups:
            ...
            if wordNext == "demà" and word != "passat":
                ...
                dayOffset -= 1
            elif wordNext == "abans-d'ahir":
                dayOffset -= 2
            elif wordNext == "abans" and wordNextNext == "ahir":
                dayOffset -= 2
            elif (wordNext == "abans" and wordNext == "abans" and
    ...
    for idx, word in enumerate(words):
        ...
        if word == "mig" and wordNext == "dia":
            hrAbs = 12
            used += 2
        elif word == "mitja" and wordNext == "nit":
            hrAbs = 0
            used += 2
        elif word == "matí":
            if not hrAbs:
            ...
            if not hrAbs:
                hrAbs = 17
            used += 2
        elif word == "tarda" and wordNext == "nit":
            if not hrAbs:
                hrAbs = 20
            used += 2
            ...
            if ':' in word:
                ...
                if remainder == "":
                    ...
                    if nextWord == "am" or nextWord == "pm":
                        remainder = nextWord
                        used += 1
                    elif wordNext == "matí" or wordNext == "matinada":
                        remainder = "am"
                        used += 1
                    elif wordNext == "tarda":
                        remainder = "pm"
